...
def is_valid_point(idx, visible, threshold=0.2):
    return (
        0 <= idx < len(visible)
        and isinstance(visible[idx], (int, float))
        and visible[idx] >= threshold
    )
...
def compute_distance_and_label(keypoints, visible, pair, cm_per_px=None):
    ...
    ptB = keypoints[b]
    # ensure valid points
    if not (is_valid_point(a, visible) and is_valid_point(b, visible)):
        return None
...
def validate_pose_geometry(all_metrics):
    ...
    # For a clean side-view bike photo, these side-to-side differences should not be extreme.
...
async def annotate_file(
    ...
    try:
        ...
        low_conf_points = [
            ...
            if not is_valid_point(i, visible, threshold=confidence_threshold)
        ]
